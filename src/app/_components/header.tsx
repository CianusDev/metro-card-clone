    ...
    <header className='w-full z-50 fixed bg-background/50 top-0 flex p-4 items-center justify-center h-20 backdrop-blur '>
        ...
        <div className='flex items-center gap-6'>
            <Link href={"/"} className='flex items-center gap-2'>
                ...
                <h1 className='font-semibold text-xl'>Metrocard-clone</h1>
            </Link>
            {menu.map((item, index) => (
                <Link key={index} href={item.href} className='flex max-md:hidden text-muted-foreground items-center gap-2'>
                    {item.name}
                </Link>
            ))}
        </div>
